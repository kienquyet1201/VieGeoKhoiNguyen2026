// ============================================================================
// LearnMatch - app-core.js (SPA Logic & Rendering - Arena Update)
// ============================================================================
    ...
    if (gameState.gems >= fee) {
        ...
        try {
            ...
            if (!waitingRooms.empty) {
                ...
                localStorage.setItem('LearnMatch_mode', 'arena');
...
function checkAndUnlockAchievements(state) {
    ...
    ACHIEVEMENTS_LIST.forEach(ach => {
        if (!state.unlockedAchievements.includes(ach.id)) {
            ...
            if (progress >= ach.target) {
                ...
                newlyUnlocked = true;
                showToast('??  m? kha danh hi?u: ' + ach.title + ' (+1 .Thnh t?u)');
            }
        }
    });
    ...
    if (newlyUnlocked) {
        saveGameState(state);
        // N?u dang ? mn hnh map, update l?i UI Profile
