...
(function () {
    ...
    async function uploadImage(file, email) {
        ...
        if (file.size > 2 * 1024 * 1024) {
            throw new Error('Ảnh đính kèm không được vượt quá 2MB.');
        }
    ...
    async function sendMessage(event) {
        ...
        try {
            ...
            if (!limit.allowed) throw new Error(`Bạn gửi tin nhắn quá nhanh. Vui lòng thử lại sau ${Math.ceil(limit.retryAfterMs / 1000)}s.`);
