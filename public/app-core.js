...
function checkOnboarding() {
    if (!gameState.grade) {
        ...
        overlay.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(10, 22, 40, 0.95); backdrop-filter: blur(10px); z-index: 9999; display: flex; justify-content: center; align-items: center;';
        
        overlay.innerHTML = `
            <div style="background: rgba(255,255,255,0.05); padding: 40px; border-radius: 24px; border: 1px solid rgba(255,255,255,0.1); box-shadow: 0 24px 48px rgba(0,0,0,0.5); text-align: center; max-width: 500px; width: 90%;">
                <i class="fa-solid fa-graduation-cap" style="font-size: 3rem; color: #1cb0f6; margin-bottom: 20px;"></i>
                <h2 style="font-size: 1.8rem; margin-bottom: 10px;">Chào mừng đến với VieGeo!</h2>
                <p style="color: rgba(255,255,255,0.7); margin-bottom: 25px;">Hãy cho chúng tôi biết bạn đang học lớp mấy để xây dựng lộ trình phù hợp nhất nhé.</p>
                <div style="display: flex; flex-direction: column; gap: 15px;">
                    <button class="bento-btn" style="font-size: 1.2rem; background: #58cc02; color: white;" onclick="selectGrade(5)">Khối Lớp 5</button>
                    <button class="bento-btn" style="font-size: 1.2rem; background: #ffc800; color: white;" onclick="selectGrade(9)">Khối Lớp 9</button>
                    <button class="bento-btn" style="font-size: 1.2rem; background: #ff4b4b; color: white;" onclick="selectGrade(12)">Khối Lớp 12</button>
                </div>
            </div>
        `;
        document.body.appendChild(overlay);
    } else {
        loadGradeData(gameState.grade);
    }
}
...
window.selectGrade = function(grade) {
    ...
    saveGameState(gameState);
    if (typeof db !== 'undefined' && sessionUser && sessionUser.email) {
        db.collection('users').doc(sessionUser.email).update({ grade: grade }).catch(()=>{});
    }
